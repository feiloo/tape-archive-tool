...
def get_all_filespaces():
    try:
        result = subprocess.run(['dsmc', 'query', 'filespace'], check=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print(result.stdout)
...
def list_archived_objects_under_path(path, ignore_error=False):
    try:
        subprocess.run(['dsmc', 'query', 'archive', '-subdir=yes', path], check=True)
    except subprocess.CalledProcessError as e:
...
def get_from_archive(name, destination):
    ...
    try:
        subprocess.run(['dsmc', 'retrieve', '-replace=no', name, destination], check=True)
...
def delete_object(name):
    ...
    try:
        subprocess.run(['dsmc', 'delete', 'archive', '-noprompt', name], check=True)
...
def print_info():
    ...
    try:
        with tempfile.NamedTemporaryFile(mode='w+', suffix='.log', prefix='app_', delete=True) as f:
            result = subprocess.run(['dsmc', 'query','systeminfo', f'-filename={f.name}'], check=True)
